...
import { fetchAllBooks, fetchBooksByCategory } from "../services/bookService";
...
export const MiniSwipper = () => {
  ...
  const getBooks = async (bookType) => {
    try {
      ...
      const booksWithCovers = (data || []).filter(
        (item) => item.cover || item.coverPic || item.cover_i
      );
      const chunks = fn(booksWithCovers);
